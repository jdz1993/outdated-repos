...
void ShowPredicted(string imgfile,char * shapefile)
{
    cv::Mat_<uchar> img=imread(imgfile,0);
    cv::Mat_<double> shape(68,2);
    ifstream fin;
    fin.open(shapefile, ifstream::in);
    char useless[1024];
    fin.getline(useless,1024);
    fin.getline(useless,1024);
    fin.getline(useless,1024);
...
int main(int argc, char* argv[])
{
    ...
    if(argc<3)
    {
        ShowPredicted("../example/helen/testset/308315761_3.jpg","../example/helen/testset/308315761_3.pts");
    }
	if (argc == 3) {
